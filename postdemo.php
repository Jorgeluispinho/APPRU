...
if(!empty($_GET['CardID'])){
    ...
    if (!mysqli_stmt_prepare($result, $sql)) {
        ...
        if ($row = mysqli_fetch_assoc($resultl)){ 
            //*****************************************************
            //Um cartão existente foi detectado para Login ou Logout
                ...
                if (!mysqli_stmt_prepare($result, $sql)) {
                    ...
                    if (!$row = mysqli_fetch_assoc($resultl)){
                        ...
                        if (!mysqli_stmt_prepare($result, $sql)) {
                            ...
                            $sql="UPDATE users SET ficha = ficha-1 WHERE CardID=$Card";
                            $result = mysqli_stmt_init($conn);
                            if (!mysqli_stmt_prepare($result, $sql)) {
                                echo "SQL_Error_Select_login1";
                                exit();
                    ...
                    //Logout
                        ...
                        if (!mysqli_stmt_prepare($result, $sql)) {
                            ...
                            mysqli_stmt_execute($result);
                            $sql = "SELECT * from logs where CardNumber = $Card";
                            ...
                            if (!mysqli_stmt_prepare($result, $sql)) {
                                ...
                                if($linha = $resultado->fetch_assoc()){ //sessao
                                    $number = $linha['SerialNumber'];
                                }    
                                $sql = "DELETE FROM logs WHERE CardNumber = $Card";
                                $result = mysqli_stmt_init($conn);
                                if (!mysqli_stmt_prepare($result, $sql)) {
                                    echo "SQL_Error_insert_logout1";
                                    exit();
                                    ...
                                    mysqli_stmt_execute($result);
                                    $sql = "DELETE FROM fila WHERE SerialNumber = $number";
